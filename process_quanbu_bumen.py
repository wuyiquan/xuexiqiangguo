...
class work_package():
    ...
    def write_to_excel(self, shunxu, data_1):
        wb = Workbook()
        ws = wb.active
        # temp_list = ["序号", "单位名称", "在职数", "学员数占在职数比例", "党员活跃学员", "党内学员", "党内学员参与度", "党外活跃学员", "党外学员", "党外学员参与度", "党内学员人均积分",
        #              "综合得分"]
        temp_list = ["序号", "单位名称", "在职数", "学员数占在职数比例", "党员活跃学员", "党内学员", "党内学员参与度", "党外活跃学员",
                     "党外学员", "党外学员参与度", "党内学员积分", "党外学员积分",
                     "党内学员人均积分", "党内学员人均积分", "所有人员人均积分", "综合得分"]
        col = 1
        row = 1
        ...
        lujing = str(self.filedir + "/" + "全部-部门" + time_ak + ".xlsx")
    ...
    def my_sort(self, mydata):
        ...
        # 按排名排序
        return sorted(list_1, key=lambda x: x[6], reverse=True)
    ...
    def every_one(self, last_data, ori, keyname):
        ...
        j = 0 if d == 0 else h/d
        k = 0 if e == 0 else i/e
        m = last_data[keyname][4]
        n = last_data[keyname][5]
        o = 0 if e == 0 else n / e
        l = 0 if d == 0 else m / d
        p = 0 if (d+e) == 0 else (m+n) / (d+e)
        # 数据格式处理
        if g != 100:
            g = round(g, 4)
        j = round(j, 4)
